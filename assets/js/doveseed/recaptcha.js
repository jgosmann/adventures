...
class ReCaptcha extends Component {
  constructor(props) {
    super(props);
    this.state = { scriptInjected: false, containerCaptured: false, recaptchaRendered: false, widgetId: null };
  ...
  render(props, state) {
    if (state.scriptInjected && state.containerCaptured && state.widgetId === null) {
      this.setState({ widgetId: grecaptcha.render(this.containerRef.current, {
  ...
  isRecaptchaScriptInjected() {
    ...
    return scripts.some((s) => s.src.match(
      /(https?:)\/\/(www\.)?google\.com\/recaptcha\/.*/i) !== null);
  }

  injectRecaptchaScript() {
    window.recaptchaLoaded = () => recaptchaInstances.forEach(x => x.setState({ scriptInjected: true }));

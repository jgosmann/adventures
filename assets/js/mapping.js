...
const showPostMap = targetId => {
  ...
  const posts = Array.from(mapElem.children).map(elem => ({
    loc: JSON.parse(elem.getAttribute("data-loc")),
    html: elem.querySelector("template").innerHTML
  }));
...
const Load = () => {
  ...
  for (let loc of locations) {
    showLocation(
      loc.getAttribute("id"),
      [
        Number(loc.getAttribute("data-lat")),
        Number(loc.getAttribute("data-long"))
      ],
  ...
  for (let route of routes) {
    showRoute(
      ...
      JSON.parse(route.getAttribute("data-route"))
    );
  }
